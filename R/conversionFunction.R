#' Conversion from BORIS CSVs to ecber compatible excel files
#'
#' This function reads in a csv directory and creates a new sub-folder to contain the converted excel files
#' @param csv_dir The path to the directory containing the CSV files to convert.
#' @return Nothing is returned but a new folder is created with the final excel files.
#' @examples
#' ConvertCSVtoExcel('./path_to_csv_dir') creates a ConvertedToExcels sub-folder
...
#' Function to convert a single BORIS file into an ecber compatible excel file
#'
#' This function takes in a CSV file that has been read in with the read.csv() function, and returns the data frame of the ecber compatible output. The ConvertCSVtoExcel function uses this function as an intermediate step before writing out the data frame to an excel.
#' @param rel The individual CSV file that has been read in with read.csv()
#' @return The final formatted data frame is returned
#' @examples
#' Boris2NoldusFileReShape(read.csv('./csv_file_name.csv))
Boris2NoldusFileReShape <- function(rel){
  #input:
  #	rel = BORIS .csv file read in through read.csv(), file that has already been read in
  #output:
  #	converted BORIS to Noldus xlsx file

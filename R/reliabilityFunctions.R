...
add_tolerance_per_event <- function(valsindffnotdff2, dff2, timechange_event, cur_confusion_matrix, tolerance = 0) {
...
createToleranceMatrix <- function(filepath1,filepath2,eventlist,tolerance=0,file_seconds=300) {
  ...
  for (events in eventlist){
    ...
    for (i in 1:length(list_of_timechange_dataframes)){
      ...
      cur_confusion_matrix = add_tolerance_per_event(valsindffnotdff2, dff2, dflist, cur_confusion_matrix, tolerance = tolerance)
    }
  }
...
createReliabilityExcel <- function(save_dir, filename1, filename2, eventlist, tol = 0, file_seconds = 300){
  cur_dir = setwd(dir)
  ...
  a = unlist(sapply(strsplit(c(filename1, filename2), split = ".xlsx"), function(x){x[1]}))
  #write it to an excel file
  addWorksheet(wb, sheetName = paste(a,collapse = 'and'))
  writeData(wb, sheet = 1, x = kappas, colNames =TRUE, rowNames= TRUE)
  addWorksheet(wb, sheetName = paste(a,collapse = 'and'))
  writeData(wb, sheet = 2, x = tol_mat0, colNames =TRUE, rowNames= TRUE)
  saveWorkbook(wb, file = output_excel_name)

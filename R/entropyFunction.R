...
#' Combined entropy output for different markov model orders
#'
#' This function takes in a list of orders for the transition matrix so we can compare detailed entropy rate information with different lag values. The entropy rates included are for all combinations of sensory signals including: TAV (Tactile, Auditory, and Visual), TAV and Affect, and Autonomy and Affect. Information about the number of transitions is also included.
#' @param file_name excel file to read in
#' @param missing_threshold default missing threshold value
#' @param order_vals This can be a vector of different order values to include in the output, or a single order value
#' @return data frame of all of the combined entropy outputs for different markov order values
combined_markov_lags <- function(file_name,
                                 missing_threshold = 0.1,
                                 order_vals = 1) {
  all_dfs = rep(NA, length(order_vals))
  for (i in 1:length(order_vals)){
    op = combined_entropy_output_more_detail(file_name, missing_threshold = missing_threshold, order = order_vals[i])
    all_dfs[i] = list(op$estimates)
  }
  final_df = data.frame(matrix(unlist(all_dfs), nrow = length(order_vals), byrow = TRUE))
  colnames(final_df) = c('SubjectID', 'EntropyRateTAV', 'TAVPercentMissing',
                         'EntropyRateTAV_Affect', 'TAV_AffectPercentMissing',
                         'EntropyRateAutonomy_Affect', 'Autonomy_AffectPercentMissing',
                         'TotalNumberOfTransitionsTAV', 'TotalNumberOfTransitionsTAV_Affect',
                         'TotalNumberOfTransitionsAutonomy_Affect', 'MaxPercentMissing')
  rownames(final_df) = paste("Order", order_vals)
  return(final_df)
}
...
#'
#' @param file_name excel file to read in
#' @param tactile_padding default tactile padding is set to 1.0
#' @param auditory_padding default auditory padding is set to 1.0
#' @param TAV_behavior_types list of all the behavior types: Tactile Auditory Visual (TAV)
#' @param affect_behavior_types list of affect behavior types including TAV and positive, negative, neutral
#' @param autonomy_behavior_types list of autonomy behavior types including autonomy support, intrusiveness, and neither
#' @param missing_threshold default missing threshold value
#' @param order Markov model order value
#' @return data frame of all of the entropy outputs for different subsets of the data including transition counts.
combined_entropy_output <- function(file_name,
                                    ...
                                    order = 1){
  fcheck_fail = FALSE
  list_TAV = ber_analyze_file(file_name, missing_threshold = missing_threshold, order = order)
  list_affect = ber_analyze_file_affect(file_name, missing_threshold = missing_threshold, order = order)
  list_affect_autonomy = ber_analyze_file_affect_and_autonomy(file_name, missing_threshold = missing_threshold, order = order)
...
#' Combined entropy output with additional information including percent missing
#'
#'This function is an extension of the combined_entropy_output function except that it outputs additional information
#' @param file_name excel file to read in
#' @param tactile_padding default tactile padding is set to 1.0
#' @param auditory_padding default auditory padding is set to 1.0
#' @param TAV_behavior_types list of all the behavior types: Tactile Auditory Visual (TAV)
...
#' @param missing_threshold default missing threshold value
#' @param order Markov model order value
#' @return data frame of all of the entropy outputs for different subsets of the data including transition counts, percent missing for each combination of sensory signals, and the maximum percent missing.
...
#' Combined entropy output for a directory with additional information
#'
#'This function is an extension of the combined_entropy_output_more_detail function except that it applies to an entire directory and creates a data frame with all the directory file information
#' @param dir_loc directory path containing excel files to read in
#' @param log_file name of log file to write output messages to
#' @param missing_threshold default missing threshold value
#' @param order Markov model order value
#' @return data frame of all entropy outputs for each file in the directory
combined_entropy_output_more_detail_dir <- function(dir_loc, log_file = paste(Sys.Date(), '-ber-logfile.txt', sep=''), missing_threshold = 0.1, order = 1){
  old_dir <- getwd()
  ...
  for (i in 1:n_files){
    ...
    capture.output(file_results <- combined_entropy_output_more_detail(individual_file, missing_threshold = missing_threshold, order = order), file=NULL)
    if(file_results$file_checks){
      files_to_check <- c(files_to_check, tail(strsplit(individual_file, '/')[[1]], 1))
      capture.output(file_results <- combined_entropy_output_more_detail(individual_file, missing_threshold = missing_threshold, order = order),
...
#' Combined entropy output for a directory
#'
#'This function is an extension of the combined_entropy_output function. It goes through an entire directory and finds the combined entropy output for each file
#' @param dir_loc directory path containing excel files to read in
#' @param log_file name of log file to write output messages to
#' @param missing_threshold default missing threshold value
#' @param order Markov model order value
...
combined_entropy_output_dir <- function(dir_loc, log_file = paste(Sys.Date(), '-ber-logfile.txt', sep=''), missing_threshold = 0.1, order = 1){
  old_dir <- getwd()
  setwd(path.expand(dir_loc))
  ...
  for (i in 1:n_files){
    individual_file <- all_files[i]
    capture.output(file_results <- combined_entropy_output(individual_file, missing_threshold = missing_threshold, order = order), file=NULL)
    if(file_results$file_checks){
      files_to_check <- c(files_to_check, tail(strsplit(individual_file, '/')[[1]], 1))
      capture.output(file_results <- combined_entropy_output(individual_file, missing_threshold = missing_threshold, order = order),
                     file = log_file, append = T)
      resultsDF[i,] <- file_results$estimates
      message(paste('Warning - See log for file : ', tail(strsplit(individual_file, '/')[[1]], 1)))
    } else{
...
#' @param tactile_padding right padding adjustment to tactile events, default is 1.0
#' @param auditory_padding right padding adjustment to auditory events, default is 1.0
...
#' @param log_file file path to write out log messages to
...
ber_analyze_file <- function(f_loc,
  ...
  if (nrow(missing) <= 0){
    #message(paste('No missing values'))
    percent_missing = 0
    total_sum_missing = 0
  } else if (nrow(missing) == 1) {
    total_sum_missing = missing$Duration_sf
  } else {
  ...
  if (order == 1){
    ...
    #we have a higher order and need to calculate it accordingly
    transition_counts <- CalcTC_Mth_Order(state_sequence[,5], 1:8, order)
    transition_matrix <- CalcTransitionMatrix(transition_counts)
    stationary_matrix <- CalcEmpStat_Mth_Order(state_sequence[,5], 1:8, order)
    entropy_rate <- CalcMarkovEntropyRate(transition_matrix, stationary_matrix)
  }
...
#' Estimate Behavioral Entropy Rate based upon Video Data Files for a Directory Including Sensory Signal Affect
#' @param dir_loc directory of files
#' @param tactile_padding right padding adjustment to tactile events
#' @param auditory_padding right padding adjustment to auditory events
#' @param behavior_types dictionary of behavior types.  Required sections, mom_auditory_types, mom_tactile_types, mom_visual_types, baby_visual_types, positive, negative, neutral, missing_types. (Positive, negative, and neutral are the affect signal types)
#' @param missing_threshold proportion of acceptable missing time
#' @param log_file file path to write out log messages to
#' @return Entropy rate estimates of an individual
...
ber_analyze_file_affect <- function(f_loc,
  ...
  if (order == 1){
    ...
    #we have a higher order and need to calculate it accordingly
    transition_counts <- CalcTC_Mth_Order(state_sequence[,5], 1:24, order)
    transition_matrix <- CalcTransitionMatrix(transition_counts)
    stationary_matrix <- CalcEmpStat_Mth_Order(state_sequence[,5], 1:24, order)
    entropy_rate <- CalcMarkovEntropyRate(transition_matrix, stationary_matrix)
  }
...
#' Estimate Behavioral Entropy Rate based upon Video Data Files for a Directory About Sensory Signals Affect and Autonomy
#' @param dir_loc directory of files
#' @param tactile_padding right padding adjustment to tactile events
#' @param auditory_padding right padding adjustment to auditory events
...
#' @param missing_threshold proportion of acceptable missing time
#' @param log_file file path for the log file to write out messages to
#' @return Entropy rate estimates of an individual
...
#' Estimate Behavioral Entropy Rate based upon Video Data for Sensory Signals Autonomy Granting and Affect
#'
#' @param f_loc file location
#' @param plot_all logical: Plot the data to observe the sequence of behaviors
#' @param plots_to_file logical: send all plots to a file
...
#' @param auditory_padding right padding adjustment to auditory events
#' @param behavior_types dictionary of behavior types.
#' Required sections: Required sections, positive, negative, neutral, autonomy support, intrusiveness, neither, missing_types
#' @param missing_threshold proportion of acceptable missing time
#' @param order Markov model order
#' @return Entropy rate estimates of an individual
ber_analyze_file_affect_and_autonomy <- function(f_loc,
  ...
  if (order == 1){
    ...
    #we have a higher order and need to calculate it accordingly
    transition_counts <- CalcTC_Mth_Order(state_sequence[,5], 1:9, order)
    transition_matrix <- CalcTransitionMatrix(transition_counts)
    stationary_matrix <- CalcEmpStat_Mth_Order(state_sequence[,5], 1:9, order)
    entropy_rate <- CalcMarkovEntropyRate(transition_matrix, stationary_matrix)
  }
